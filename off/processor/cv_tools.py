...
def cv2_to_stream(image, kind="jpg", quality=95):
    ...
    retval, buff = cv2.imencode('.' + kind, image,
...
def cv2_to_file(image, target_file, quality=95):
    ...
    return cv2.imwrite(target_file, image, [cv2.IMWRITE_JPEG_QUALITY, quality,
                                            cv2.IMWRITE_PNG_COMPRESSION, 2])
...
def blankfield_generate_flat(bkg_in):
    ...
    print("hnew: {}, snew: {}, vnew: {}".format(h_new.min(), s_new.min(),
                                                v_new.min()))
...
def entropy(image, subdivisions=10):
    ...
    subregions = [image[stride_r*i:min(stride_r*(i+1), h),
                        stride_c*j:min(stride_c*(j+1), w), :]
