...
def main():
  ...
  parser.add_argument('-a', '--algorithm',
                      ...
                      choices=['SURF', 'SIFT'],
                      default='SURF')
  parser.add_argument('-c1', '--crop-im1',
                      help="Crop coordinates l,t,r,b for image 1",
                      default=None)
  parser.add_argument('-c2', '--crop-im2',
                      help="Crop coordinates l,t,r,b for image 2",
                      default=None)

  args = parser.parse_args()

  ## Load images.
  image1_full = cv2.imread(args.im1)
  image2_full = cv2.imread(args.im2)

  if args.crop_im1 is None:
      image1 = image1_full
  else:
      left, top, right, bottom = map(int, args.crop_im1.split(','))
      image1 = image1_full[top:bottom, left:right].copy()

  if args.crop_im2 is None:
      image2 = image2_full
  else:
      left, top, right, bottom = map(int, args.crop_im2.split(','))
      image2 = image2_full[top:bottom, left:right].copy()
...
def draw_correspondences(image1, image2, points1, points2):
  ...
  for (x1, y1), (x2, y2) in zip(np.int32(points1), np.int32(points2)):
    color = 255*np.array(colorsys.hls_to_rgb(np.random.rand(), 0.62, 1))
    cv2.line(image, (x1, y1), (x2+w1, y2), tuple(color.round().astype(int)), lineType=cv2.CV_AA)
