...
class MotorLink(motor_api.BaseMotorLink):
    ...
    def global_command_byte(self, command, value):
        to_send = struct.pack('<BB', motor_api.global_register_offset[command], int(value))
        self.send(to_send)

    def global_command_word(self, command, value, unsigned=False):
        word_spec = ['h', 'H']
        spec = '<B' + word_spec[unsigned]
        to_send = struct.pack(spec, motor_api.global_register_offset[command], int(value))
        self.send(to_send)
    ...
    def global_command_sequence(self, command, values):
        ...
        if length <= 32:
            ...
            to_send = struct.pack(spec, motor_api.global_register_offset[command], length, *sequence)
            self.send(to_send)
        else:
