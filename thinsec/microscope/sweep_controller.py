...
motor_indices = { motor: params['index'] for motor, params
                  in get_hardware_config().items() }
...
class SweepController(object):
    ...
    def start(self, parameters):
        ...
        dir_gen = ( (kind, idx_angle) for kind, angles
                   in angles_spec.items() for idx_angle, angle
                   in enumerate(angles) )
        dirs = [ pth.join(kind, str(idx_angle)) for kind, idx_angle
                in dir_gen ]
        self.cameras['cam0'].set_sweep_settings({'name': cname,
                                                 'directories': dirs,
                                                 'active': True})
        self.focus_reset()
    ...
    def reset(self, event):
        ...
        map_row = default_map_position(0, limit_row * self.strides['y'],
                                       0, limit_row)
        map_col = default_map_position(0, limit_column * self.strides['x'],
                                       0, limit_column)
        ...
        if isinstance(self.strides['pol'], list):
            ...
            half_travel = default_map_position(0, self.strides['pol']*xpl_steps,
                                               0, limit_angle)
        ...
        self.map_position = {'x': map_col, 'y': map_row,
                             'ana': map_ana, 'pol': map_pol}
        ...
        self.axes_stops = {'x': self.num_cols, 'y': self.num_rows,
                           'ana': self.num_angles}
    ...
    def prepare_motors(self, event):
        ...
        for motor in motor_indices:
            self.motor_exec.set_acceleration(motor,
                                             self.accelerations[motor], True)
            self.motor_exec.set_max_speed(motor,
                                          self.max_speeds[motor], True)
    ...
    def move(self, event):
        ...
        phy_parts = [ list(self.angle_pos_tag[ang]) + [row, col]
                     for ang, row, col in log_parts ]
        filenames = [ self.filename_format.format(kind, ang, row, col) for
                     kind, ang, row, col in phy_parts ]
        exposures = [ self.exposure_time_ms[p[0]] for p in phy_parts ]
        self.cameras['cam0'].configure_fastaxis_scan({'seq': filenames,
                                                      'expo': exposures})
    ...
    def camera_event_dispatch(self, data):
        ...
        if self.fsm.isstate('idle'):
            ...
            if keys[1] in data:
                debug_log("CAM", "error saving image at kind:", s_parts['kind'],
                          "angle:", s_parts['ana'], "row:", s_parts['y'],
                          "col:", s_parts['x'])
                self.fsm.trigger("stop")
    ...
    def focus_set(self):
        ...
        point = (self.motor_exec.get_position('x'),
                 self.motor_exec.get_position('y'),
                 self.motor_exec.get_position('z'))
    ...
    def focus_build_map(self, spread=None):
        ...
        if len(self.focus_table) > 0:
            ...
            self.focus_map = interpolate.Rbf(x, y, z, epsilon=spread,
                                             function='gaussian')
            self.do_focus = True
        else:
...
def sweep_gen(steps_axis_major, steps_axis_minor, steps_axis_fast):
    ...
    dirs_axis_minor, dirs_axis_fast = gen_direction_steps(steps_axis_major,
                                                          steps_axis_minor, [1, 1])
    ...
    if steps_axis_major > 1:
        ...
        indices_axis_minor = np.hstack((indices_axis_minor,
                                        np.hstack(cycles_axis_minor) ))
