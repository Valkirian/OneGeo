...
class MotorLink(motor_api.BaseMotorLink):
    ...
    def motor_command_byte(self, motor, command, value):
        ...
        to_send = struct.pack('<BB', self.register_address(motor, command),
                            int(value))
        self.send(to_send)

    def motor_command_word(self, motor, command, value):

        to_send = struct.pack('<Bh', self.register_address(motor, command),
                            int(value))
        self.send(to_send)

    def motor_command_int(self, motor, command, value):

        to_send = struct.pack('<Bi', self.register_address(motor, command),
                            int(value))
        self.send(to_send)

    def global_command_byte(self, command, value):

        to_send = struct.pack('<BB', motor_api.global_register_offset[command],
                            int(value))
        self.send(to_send)
    ...
    def global_command_word(self, command, value, unsigned=False):
        ...
        to_send = struct.pack(spec, motor_api.global_register_offset[command],
                              int(value))
        self.send(to_send)
    ...
    def send(self, to_send):
        ...
        if self.debug:
            print "UART send:", ', '.join("{:02X}".format(ord(c))
                                          for c in to_send)
        print "UART send:", ', '.join("{:02X}".format(ord(c))
                                        for c in to_send)
