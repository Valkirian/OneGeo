...
def run_kind(kind, max_idx, base_dir=".", threads=1, roi=-1):
    ...
    out_file = pth.join(pth.abspath(base_dir), kind, str(roi),
                        "focus-curve-moment-{}.json".format(max_idx))
...
def image_focus_worker(filename, z_pos, max_idx, base_dir, center_roi):
    ...
    img = ( img_rgb if center_roi == -1 else
           crop_center_chunk(img_rgb, center_roi) )
