...
def main():
    ...
    for img_file in targets:
        ...
        while not converged:
            ...
            if s_width not in img1_features:

                debug_log("Gather features of reference image", basename_ref,
                          "with w =", s_width)
                img1_bands = get_image_bands(img1_subg, s_width)
                kp1, des1 = get_features_on_bands(img1_subg, img1_bands, sift)

                if kp1 is None:
                    debug_log("No features detected. Increasing bands width")
                    s_width = get_next_iter(s_width, match_stats)
                    continue

                img1_features[s_width] = (kp1, des1)
            else:
                debug_log("Using features of reference", basename_ref,
                          "with w =", s_width)

            ref_package = img1_features[s_width]
            ...
            print(warp_matrix)
            success = (warp_matrix is not None )#and is_good_matrix(warp_matrix, 3e-3))
            converged = success
...
def process_command_line():
    ...
    parser.add_argument('-c', "--contrast-threshold", type=float, default=0.1,
            help="Initial contrast threshold parameter for SIFT")
