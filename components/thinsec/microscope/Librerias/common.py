...
def log_line(*args, **kwargs):
    ...
    now = get_utc_now_ms()
    line = "{}: {}{}".format(integer_separate(now), ' '.join(map(str, args)), newline)
    return line
...
class DebugLog(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        delta = now - self.last
        self.last = now
        line = "+{} ms: {}{}".format(integer_separate(delta), ' '.join(map(str, args)), newline)
...
def parse_image_grid_list(files_in, as_dict=False):
    ...
    file_table = [(f, pth.splitext(pth.basename(f))[0].split('_')) for f in files]
    ...
    row_cells = [ [f[0] for f in file_table if f[1][0] == row] for row in rows ]
