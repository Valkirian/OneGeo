...
class AccelStepper
{
    /// For interrupt handling: static object counter and reference table.
    static unsigned int _object_count;
    static AccelStepper* refs[MAX_COUNT];
...
public:
    ...
    /// \brief Position states depending on the status of the endstops
    typedef enum
    {
	AT_MIN,
	AT_MAX,
	INSIDE
    } EndstopPositionState;
    ...
    ~AccelStepper() {
        //TODO this is not complete; the refs array should be a dictionary instead;
        //  however, destroying objects of this class is an unusual case
        --_object_count;
    }

    /// For interrupt handling: global interrupt handler as a dispatcher.
    static void interrupt_handler() {
        for(int i=0; i<_object_count; ++i) {
            refs[i]->_on_endstop_min();
            refs[i]->_on_endstop_max();
        }
    }
    ...
    /// If called as often as run(), returns true if the motor just finished motion
    /// \return true if the motor just finished stepping.
    boolean runFinished();

    /// If called as often as run(), returns true if the motor just started motion
    /// \return true if the motor just started stepping.
    boolean runStarted();

    /// Updates the value of _last_value; To be run after runFinished or runStarted.
    void update_last_done(void);
    ...
    /// Sets the "monitoring" speed, for which the stepsToEndSpeed() function
    /// returns the amount of steps remaining to attain.
    /// \param[in] speed The desired monitoring speed in steps per second.
    ...
    /// Steps to break from the end speed to zero.
    /// \return distance required to deccelerate from the end speed to zero.
    unsigned int   stepsToBrakeEndSpeed();

    /// Steps to break from the instantaneous speed to zero.
    /// \return distance required to deccelerate completely.
    long           breakingDistance();
    ...
    /// The amount of steps remaining for the speed to reach the set
    //  monitoring speed.
    /// \return Steps before attaining monitoring speed
    /// in steps. Always positive (independent of direction).
    long    stepsToEndSpeed();
    ...
    /// Indicates whether the motor is currently undergoing motion
    boolean  done(void);

    /// Called as setter of the _backlash variable
    /// \param[in] n_backlash the new amount of backlash_
    void set_backlash(unsigned int);
    ...
    /// Configures up to two endstop pins, and the absolute position assigned to
    /// the one marked as "min".
    /// \param[in] min_pin The pin number of a digital input to use as "min" endstop
    /// \param[in] max_pin The pin number of a digital input to use as "max" endstop
    /// \param[in] min_pos The absolute position assigned to the motor when the "min"
    //                     endstop is triggered (its logic level falls to zero)
    void configure_endstops(int min_pin, int max_pin, int min_pos);

    /// Returns true if an endstop has been reached
    bool hit_endstop();
...
protected:
    ...
    /// Implements the endstop action of the "min" endstop
    void _on_endstop_min();

    /// Implements the endstop action of the "max" endstop
    void _on_endstop_max();
...
private:
    ...
    float          _speed_sq;      // (Steps per second)^2

    /// The maximum permitted speed in steps per second. Must be > 0.
    float          _maxSpeed;      // Steps per second

    /// The target end speed (usually zero).
    float          _endSpeed;      // Steps per second
    float          _endSpeed_sq;   // (Steps per second)^2

    /// Steps to break from the end speed to zero.
    unsigned int   _stepsToBrakeEndSpeed;

    /// The acceleration to use to accelerate or decelerate the motor in steps
    /// per second per second. Must be > 0
    float          _acceleration;
    float          _twoa;      // Precomputed 2*_acceleration
    ...
    /// The amount of steps to compensate for backlash
    ...
    /// The control objective: is it position or speed
    bool _objective_is_position;
    ...
    /// Previous direction motor was spinning in
    boolean _last_direction; // 1 == CW

    /// Previous motion state
    boolean _last_done;

    /// Endstop pins
    int _pin_endstop_min, _pin_endstop_max;

    /// Endstop collision flag
    bool _hit_endstop;

    /// The position to set when the min endstop is hit
    int _min_endstop_position;

    /// The position state as per the endstops
    EndstopPositionState _endstop_state;
